...
contract OliveV3 is IOlive, Allowed {
    ...
    function getMaxAvailabilityPool() public view returns (ILendingPool, uint256) {
        ...
        IAssetManager lpManager = IAssetManager(_lpManager);
    ...
    function _mintLP(address _token, uint256 _borrowed) internal returns (uint256) {
        ...
        IAssetManager glpManager = IAssetManager(_lpManager);
    ...
    function deleverage(uint16 _toLeverage) external override returns (bool) {
        address user = msg.sender;
        return _deleverageForUser(user, _toLeverage);
    }

    function _deleverageForUser(address  _user, uint16 _toLeverage) internal returns (bool) {
        require((_toLeverage >= 1 && _toLeverage <= 5), "OLV: No leverage");
